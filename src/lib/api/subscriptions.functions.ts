...
export const adminLogin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Store the token server-side for validation
...
// In-memory token store (resets on server restart; acceptable for admin dashboard)
...
export const saveSubscription = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    // TODO: Add rate limiting here in production to prevent spamming subscription creation.
...
export const confirmStripeSession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Retrieve default payment method to get masked details
    ...
    // Update database status & payment details
